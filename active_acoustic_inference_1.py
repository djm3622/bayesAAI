...
from scipy.signal import welch, butter, bilinear_zpk, zpk2sos, sosfilt
...
DRIVE_AMP     = 0.05    # Sine amplitude 0.0–1.0 (digital full scale)
BABBLE_DBFS   = -20.0   # A-weighted babble level in dBFS  (e.g. -20 = moderately loud)
SAMPLE_RATE   = 44100    # Hz — standard audio rate
N_STEPS       = 150       # Number of probe frequencies
T_PER_STEP    = 0.2      # Lock-in integration time per frequency (seconds)
F_MIN         = 35       # Hz — lowest frequency to probe
...
# ═══════════════════════════════════════════════════════════════════════════════
# 2b. A-WEIGHTING AND BABBLE NOISE HELPERS
# ═══════════════════════════════════════════════════════════════════════════════

def design_a_filter(fs):
    """
    Design a digital A-weighting filter (IEC 61672-1:2013).

    A-weighting approximates the sensitivity of human hearing vs. frequency.
    The filter has 0 dB response at 1000 Hz by convention.

    Returns second-order sections (SOS) for use with scipy.signal.sosfilt.
    """
    # Characteristic frequencies of the analog A-weighting prototype
    f1, f2, f3, f4 = 20.598997, 107.65265, 737.86223, 12194.217
    w1, w2, w3, w4 = [2 * np.pi * f for f in [f1, f2, f3, f4]]

    # Analog prototype: 4 zeros at origin, 6 poles (double at f1 and f4)
    z_a = np.zeros(4)
    p_a = np.array([-w1, -w1, -w2, -w3, -w4, -w4])
    k_a = (w4 * w1) ** 2
    ...
    # Normalize so that |H(1000 Hz)| = 1  (0 dB at 1 kHz)
    t_norm = np.arange(2 * fs) / fs
    x_norm = np.sin(2 * np.pi * 1000 * t_norm)
    y_norm = sosfilt(sos, x_norm)
    rms_in  = np.sqrt(np.mean(x_norm[fs:]**2))
    rms_out = np.sqrt(np.mean(y_norm[fs:]**2))
    sos[0, :3] *= rms_in / rms_out   # scale b0,b1,b2 of first section

    return sos


def generate_babble(N, fs, target_rms_A, sos_a, seed=None):
    """
    Generate speech-spectrum-shaped noise as a babble surrogate.

    Procedure:
      1. White noise → bandpass 80–8 kHz  (speech frequency range)
      2. Mix with low-passed version to add low-frequency emphasis
         (approximates the vowel-dominated spectrum of crowd babble)
      3. Scale so that the A-weighted RMS equals target_rms_A

    Args:
        N            : number of samples
        fs           : sample rate (Hz)
        target_rms_A : desired A-weighted RMS level (linear, not dB)
        sos_a        : A-weighting SOS from design_a_filter()
    ...
    # Bandpass to speech range
    bp = butter(4, [80.0, 7500.0], btype='bandpass', fs=fs, output='sos')
    noise = sosfilt(bp, noise)

    # Blend with low-passed copy for low-mid emphasis (~speech formants)
    lp = butter(2, 800.0, btype='low', fs=fs, output='sos')
    noise = 0.55 * sosfilt(lp, noise) + 0.45 * noise

    # Scale to target A-weighted RMS
    rms_A = np.sqrt(np.mean(sosfilt(sos_a, noise) ** 2))
    if rms_A > 0:
        noise *= target_rms_A / rms_A

    return noise.astype(np.float32)


# ═══════════════════════════════════════════════════════════════════════════════
# 2c. REAL LOCK-IN MEASUREMENT  (microphone + speaker via sounddevice)
# ═══════════════════════════════════════════════════════════════════════════════

class RealLockin:
    """
    Real lock-in measurement using a stereo speaker pair and a microphone.

    Output routing:
      Left  channel (ch 0) — babble noise  (masks the probe tone)
      Right channel (ch 1) — sinusoidal probe

    The babble noise is speech-spectrum-shaped and set to a calibrated
    A-weighted level (BABBLE_DBFS).  The lock-in rejects it completely
    because it is incoherent with the reference sinusoid.

    SNR is reported as:  20·log₁₀( RMS_sine / RMS_A-weighted_babble )

    Call calibrate_noise() once at startup to measure the ambient noise
    floor used by the GP likelihood.
    """

    def __init__(self, fs=44100, drive_amplitude=0.05,
                 babble_level_dBFS=-20.0,
        ...
        self._noise_psd_estimate = 1e-6   # updated by calibrate_noise()

        # A-weighting filter
        self._sos_a = design_a_filter(fs)

        # Target A-weighted RMS for babble (converted from dBFS)
        self._babble_rms_A = 10 ** (babble_level_dBFS / 20.0)

        # Pre-compute SNR (fixed once amplitude and babble level are set)
        rms_sine = drive_amplitude / np.sqrt(2)
        self.snr_dB = 20 * np.log10(rms_sine / self._babble_rms_A)

        print(f'Babble level : {babble_level_dBFS:.1f} dBFS  '
              f'(A-weighted RMS = {self._babble_rms_A:.4f})')
        print(f'Sine RMS     : {rms_sine:.4f}  '
              f'(amplitude = {drive_amplitude:.3f})')
        print(f'SNR          : {self.snr_dB:+.1f} dB  '
              f'(sine RMS / A-weighted babble RMS)\n')
    ...
    def measure(self, f, T=3.0):
        """
        Stereo output: left = babble noise, right = sinusoidal probe.
        Records microphone and returns complex H(f) via lock-in detection.

        Lock-in detection math:
            Play  (right): s(t) = A · sin(2πft)
            Record (mic):  x(t) = |H|·A · sin(2πft + φ) + babble(t) + noise(t)
            Extract: I = 2·mean(x·sin(2πft)) / A  →  |H|·cos(φ)
                     Q = 2·mean(x·cos(2πft)) / A  →  |H|·sin(φ)
            Babble is incoherent with reference → averages to zero.
        """
        N = int(T * self.fs)
        t = np.arange(N) / self.fs

        # Right channel: sinusoidal probe
        sine   = (self.A * np.sin(2 * np.pi * f * t)).astype(np.float32)

        # Left channel: babble noise at calibrated A-weighted level
        babble = generate_babble(N, self.fs, self._babble_rms_A, self._sos_a)

        # Stereo output array: shape (N, 2)
        out = np.column_stack([babble, sine]).astype(np.float32)

        # Play stereo, record 1 input channel (microphone)
        rec = sd.playrec(out, samplerate=self.fs,
                         channels=1, dtype='float32',
                         device=(self.in_d, self.out_d))
        sd.wait()
        x = rec[:, 0].astype(float)
